...
void MeanShift::CalWeight(const cv::Mat &window, cv::Mat &target_model, 
                    cv::Mat &target_candidate, cv::Rect &rec)
{
    int rows = rec.height;
    int cols = rec.height; // rec.width;
    ...
    for(int i=0; i<rows; i++)
    {
        for(int j=0; j<cols; j++)
        {
            ...
            int64_t fixed_div = DIV64(target_model.at<int>(0, bin_value), target_candidate.at<int>(0, bin_value));
            int64_t fixed_sqrt = FIXEDSQRT(fixed_div);
            weight[i][j] = MULT64(INT2FIXED(1), fixed_sqrt);
        }
    }
    ...
    for(int k = 1; k < 3;  k++)
    {
        for(int i=0; i<rows; i++)
        {
            for(int j=0; j<cols; j++)
            {
                ...
                int64_t fixed_div = DIV64(target_model.at<int>(k, bin_value), target_candidate.at<int>(k, bin_value));
                int32_t fixed_sqrt = FIXEDSQRT(fixed_div);
                // weight.at<int>(i,j) = static_cast<int>(MULT64(weight.at<int>(i,j), (fixed_sqrt)));
                weight[i][j] = MULT64(weight[i][j], fixed_sqrt);
                // if (count == 1)
...
cv::Rect MeanShift::track(const cv::Mat &next_frame, const cv::Mat &mult)
{
    ...
    for(int iter=0; iter<cfg.MaxIter; iter++)
    {
        ...
        for(int i=0; i<58; i++)
        {
            ...
            for(int j=0; j<58; j++)
            {
                if (mult.at<int>(i,j)) {
                    ...
                    // delta_x += static_cast<int>(MULT64(norm_j, weight.at<int>(i,j)));
                    // delta_y += static_cast<int>(MULT64(norm_i, weight.at<int>(i,j)));
                    delta_x += /*static_cast<int>*/(MULT64(norm_j, weight[i][j]));
                    delta_y += /*static_cast<int>*/(MULT64(norm_i, weight[i][j]));
                    sum_wij += /*static_cast<int>*/(weight[i][j]);
                }
                norm_j += icentre;
            }
            norm_i += icentre;
        }
        // std::cout << "delta_x/sum_wij: " << (FIXED2FLOAT(delta_x)/FIXED2FLOAT(sum_wij)) << std::endl;
        ...
        next_rect.x += FIXED2INT(MULT(DIV64(delta_x, sum_wij), centre));
        next_rect.y += FIXED2INT(MULT(DIV64(delta_y, sum_wij), centre));
